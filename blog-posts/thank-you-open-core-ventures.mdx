...
For Kubetail, Catalyst was a game-changer.

Previous to working on Kubetail I co-founded a startup, [Octopart](https://octopart.com), that went through Y Combinator's W07 batch. As far as startups go, we didn't do badly so when I started working on Kubetail I followed a similar approach to what we'd done previously. I focused on building an MVP and as soon as it was ready, I posted it to Hacker News (HN). Luckily, the HN post reached the front page for a few hours and we ended up with a couple of hundred GitHub stars and a small number of real users (~10).
