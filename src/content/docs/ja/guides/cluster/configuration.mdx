...
Dashboard は UI をホストし、ブラウザーからクラスタへのリクエストを中継する Web サーバーです。クラスタ環境では `kubetail` namespace の Deployment として動作します。
...
Cluster API は、Dashboard から各ノードで動作する Cluster Agent インスタンスへリクエストを中継するバックエンドサーバーです。クラスタ内では `kubetail` namespace の Deployment として動作します。
...
Cluster Agent は各ノードで動作し、ノードのファイルシステムからコンテナのログファイルを直接読み取る小さな Rust 製プログラムです。クラスタ内では `kubetail` namespace の DaemonSet として動作します。
