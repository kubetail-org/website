...
インストールが終われば、すぐにログの tail を始められます。`kubetail` CLI ツールは認証に kubeconfig ファイルを使うため、`kubectl` と同じクラスターにアクセスできます。すべてのコマンド一覧は [CLI リファレンス](/ja/reference/cli) を参照してください。

* **Web ダッシュボード**

    [`serve`](/ja/reference/cli#kubetail-serve) コマンドを使って Web ダッシュボードを起動します（[http://localhost:7500](http://localhost:7500) で開きます）:
...
* **ターミナル**
    ...
    [`logs`](/ja/reference/cli#kubetail-logs) コマンドを使ってログをターミナルへ直接ストリームします:
...
* **設定**
    ...
    [`config init`](/ja/reference/cli#kubetail-config) コマンドを使って、`~/.kubetail/config.yaml` にローカル設定ファイルを作成します:
...
* **高度な機能**
    ...
    [`cluster install`](/ja/reference/cli#kubetail-cluster) コマンドでクラスター側のリソースをインストールし、検索などの高度な機能を有効にします:
...
* [Cluster](/ja/guides/cluster/installation)
