...
既定では、chart は dashboard のセッション署名キーと暗号化キー（`KUBETAIL_DASHBOARD_SESSION_SIGNING_KEY1` と `KUBETAIL_DASHBOARD_SESSION_ENCRYPTION_KEY1`）を自動生成し、Kubernetes Secret に保存するため、アップグレード後も保持されます。
...
| `kubetail.secrets.KUBETAIL_DASHBOARD_SESSION_SIGNING_KEY1` | string | `null` | Hex エンコードされた dashboard セッション署名キー（`null` の場合は自動生成） |
| `kubetail.secrets.KUBETAIL_DASHBOARD_SESSION_ENCRYPTION_KEY1` | string | `null` | Hex エンコードされた dashboard セッション暗号化キー（`null` の場合は自動生成） |
| `kubetail.secrets.KUBETAIL_DASHBOARD_SESSION_SIGNING_KEY2` | string | `null` | ローテーション用の前スロット署名キー（Hex エンコード、自動生成なし） |
| `kubetail.secrets.KUBETAIL_DASHBOARD_SESSION_ENCRYPTION_KEY2` | string | `null` | ローテーション用の前スロット暗号化キー（Hex エンコード、自動生成なし） |
...
| `kubetail.dashboard.runtimeConfig` | map | *see values.yaml* | Dashboard のランタイム設定（[Dashboard リファレンス](/ja/reference/dashboard) を参照） |
...
| `kubetail.dashboard.podTemplate.imagePullSecrets` | array | `[]` | Image pull secret |
...
| `kubetail.clusterAPI.runtimeConfig` | map | *see values.yaml* | Cluster API のランタイム設定（[Cluster API リファレンス](/ja/reference/cluster-api) を参照） |
...
| `kubetail.clusterAPI.podTemplate.imagePullSecrets` | array | `[]` | Image pull secret |
...
| `kubetail.clusterAgent.runtimeConfig` | map | *see values.yaml* | Cluster Agent のランタイム設定（[Cluster Agent リファレンス](/ja/reference/cluster-agent) を参照） |
...
| `kubetail.clusterAgent.podTemplate.imagePullSecrets` | array | `[]` | Image pull secret |
...
| `kubetail.clusterAgent.rbac.name` | string | `null` | RBAC リソース名を上書き |
| `kubetail.clusterAgent.rbac.annotations` | map | `{}` | 追加の RBAC annotation |
| `kubetail.clusterAgent.rbac.labels` | map | `{}` | 追加の RBAC label |
