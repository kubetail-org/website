...
Por defecto, el chart genera automaticamente las claves de firma y cifrado de la sesion del dashboard (`KUBETAIL_DASHBOARD_SESSION_SIGNING_KEY1` y `KUBETAIL_DASHBOARD_SESSION_ENCRYPTION_KEY1`) y las guarda en un Secret de Kubernetes para que se conserven en los upgrades.
...
| `kubetail.secrets.KUBETAIL_DASHBOARD_SESSION_SIGNING_KEY1` | string | `null` | Clave de firma de la sesion del dashboard codificada en hex (se genera automaticamente si es `null`) |
| `kubetail.secrets.KUBETAIL_DASHBOARD_SESSION_ENCRYPTION_KEY1` | string | `null` | Clave de cifrado de la sesion del dashboard codificada en hex (se genera automaticamente si es `null`) |
| `kubetail.secrets.KUBETAIL_DASHBOARD_SESSION_SIGNING_KEY2` | string | `null` | Clave de firma del slot anterior para rotacion codificada en hex (sin generacion automatica) |
| `kubetail.secrets.KUBETAIL_DASHBOARD_SESSION_ENCRYPTION_KEY2` | string | `null` | Clave de cifrado del slot anterior para rotacion codificada en hex (sin generacion automatica) |
...
| `kubetail.dashboard.runtimeConfig` | map | *see values.yaml* | Configuracion de ejecucion del Dashboard (vea la [referencia del Dashboard](/es/reference/dashboard)) |
...
| `kubetail.dashboard.podTemplate.imagePullSecrets` | array | `[]` | Secretos de pull de imagen |
...
| `kubetail.clusterAPI.runtimeConfig` | map | *see values.yaml* | Configuracion de ejecucion de Cluster API (vea la [referencia de Cluster API](/es/reference/cluster-api)) |
...
| `kubetail.clusterAPI.podTemplate.imagePullSecrets` | array | `[]` | Secretos de pull de imagen |
...
| `kubetail.clusterAgent.runtimeConfig` | map | *see values.yaml* | Configuracion de ejecucion de Cluster Agent (vea la [referencia de Cluster Agent](/es/reference/cluster-agent)) |
...
| `kubetail.clusterAgent.podTemplate.imagePullSecrets` | array | `[]` | Secretos de pull de imagen |
...
| `kubetail.clusterAgent.rbac.name` | string | `null` | Sobrescribir los nombres de los recursos RBAC |
| `kubetail.clusterAgent.rbac.annotations` | map | `{}` | Anotaciones RBAC adicionales |
| `kubetail.clusterAgent.rbac.labels` | map | `{}` | Labels RBAC adicionales |
