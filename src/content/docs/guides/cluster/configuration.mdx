...
The Dashboard is the web server that hosts the UI and proxies requests from the browser to the cluster. In the cluster environment it runs as a Deployment in the `kubetail` namespace.
...
The Cluster API is is the backend server that proxies requests from the Dashboard to the Cluster Agent instances running on each node.  In the cluster it runs as a Deployment in the `kubetail` namespace.
...
The Cluster Agent is the small, Rust-based program that runs on each node and reads container log files directly from the node filesystem. In the cluster it runs as a DaemonSet in the `kubetail` namespace.
