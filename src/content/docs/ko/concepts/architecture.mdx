...
설치 방법은 [데스크톱 설치 가이드](/ko/guides/desktop/installation)를 참고하세요.

### 클러스터

Kubetail 전체 스택은 Helm 또는 YAML 매니페스트를 사용해 클러스터 내부의 Kubernetes 리소스로 설치할 수 있습니다. 이 토폴로지에서는 Dashboard Server가 `kubetail` namespace의 Deployment로 실행되며, 브라우저에서 `kubectl port-forward`, `kubectl proxy`, 또는 ingress 리소스를 통해 접근합니다. 이는 팀이 공유하는 환경이나, 로컬 CLI에 의존하지 않고 항상 켜져 있는 지속적인 접근이 필요한 경우에 선호되는 방식입니다.

설치 방법은 [클러스터 설치 가이드](/ko/guides/cluster/installation)를 참고하세요.
...
Docker 배포 옵션에 대한 자세한 내용은 [Docker 소개](/ko/guides/docker/introduction)를 참고하세요.
...
전체 명령 및 플래그 목록은 [CLI 레퍼런스](/ko/reference/cli)를 참고하세요.
...
설정 옵션은 [Dashboard 레퍼런스](/ko/reference/dashboard)를 참고하세요.
...
설정 옵션은 [Cluster API 레퍼런스](/ko/reference/cluster-api)를 참고하세요.
...
설정 옵션은 [Cluster Agent 레퍼런스](/ko/reference/cluster-agent)를 참고하세요.
...
<Aside type="tip">
각 토폴로지의 성능상 차이를 더 자세히 보려면 [Performance](/ko/concepts/performance) 페이지를 참고하세요.
</Aside>
