...
Par defaut, le chart genere automatiquement les cles de signature et de chiffrement de la session du dashboard (`KUBETAIL_DASHBOARD_SESSION_SIGNING_KEY1` et `KUBETAIL_DASHBOARD_SESSION_ENCRYPTION_KEY1`) et les stocke dans un Secret Kubernetes afin qu'elles soient conservees lors des mises a niveau.
...
| `kubetail.secrets.KUBETAIL_DASHBOARD_SESSION_SIGNING_KEY1` | string | `null` | Cle de signature de la session du dashboard encodee en hex (generee automatiquement si `null`) |
| `kubetail.secrets.KUBETAIL_DASHBOARD_SESSION_ENCRYPTION_KEY1` | string | `null` | Cle de chiffrement de la session du dashboard encodee en hex (generee automatiquement si `null`) |
| `kubetail.secrets.KUBETAIL_DASHBOARD_SESSION_SIGNING_KEY2` | string | `null` | Cle de signature de l'emplacement precedent pour la rotation, encodee en hex (pas de generation automatique) |
| `kubetail.secrets.KUBETAIL_DASHBOARD_SESSION_ENCRYPTION_KEY2` | string | `null` | Cle de chiffrement de l'emplacement precedent pour la rotation, encodee en hex (pas de generation automatique) |
...
| `kubetail.dashboard.podTemplate.imagePullSecrets` | array | `[]` | Secrets de pull d'image |
...
| `kubetail.clusterAPI.podTemplate.imagePullSecrets` | array | `[]` | Secrets de pull d'image |
...
| `kubetail.clusterAgent.podTemplate.imagePullSecrets` | array | `[]` | Secrets de pull d'image |
...
| `kubetail.clusterAgent.rbac.name` | string | `null` | Remplacer les noms des ressources RBAC |
| `kubetail.clusterAgent.rbac.annotations` | map | `{}` | Annotations RBAC supplementaires |
| `kubetail.clusterAgent.rbac.labels` | map | `{}` | Labels RBAC supplementaires |
