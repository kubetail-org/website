...
- **Sur votre poste** — utilisez la CLI `kubetail` pour lancer un serveur de dashboard local qui s'authentifie auprès de votre cluster à l'aide de votre fichier kubeconfig. Aucune installation côté cluster n'est requise. (Voir [Exécuter sur poste > Installation](/fr/guides/desktop/installation))
- **Dans votre cluster** — déployez le dashboard comme workload Kubernetes à l'aide de Helm ou de manifests YAML, puis accédez-y via `kubectl port-forward`, `kubectl proxy` ou une ressource ingress. (Voir [Exécuter dans le cluster > Installation](/fr/guides/cluster/installation))
...
|---------------------------------------------------------------------|---------------------------------------|-------------------------------------------------------------------|
| [Kubernetes API (Basic)](/fr/guides/dashboard/basic-features)     | Non                                   | Tail en temps réel, filtrage de base                              |
| [Kubetail API (Advanced)](/fr/guides/dashboard/advanced-features) | Oui                                   | Toutes les fonctions de base + tailles de fichiers de logs, horodatages du dernier événement, recherche |
...
<Aside type="tip">
Pour une explication plus détaillée du fonctionnement de chaque pipeline, consultez la page [Architecture](/fr/concepts/architecture#log-delivery-pipelines).
</Aside>
