...
Dashboard 是承载 UI 并将浏览器请求代理到集群的 Web 服务器。在集群环境中，它作为 `kubetail` namespace 中的 Deployment 运行。
...
Cluster API 是后端服务器，用于将 Dashboard 的请求代理到运行在每个节点上的 Cluster Agent 实例。在集群中，它作为 `kubetail` namespace 中的 Deployment 运行。
...
Cluster Agent 是一个运行在每个节点上的小型 Rust 程序，它直接从节点文件系统读取容器日志文件。在集群中，它作为 `kubetail` namespace 中的 DaemonSet 运行。
