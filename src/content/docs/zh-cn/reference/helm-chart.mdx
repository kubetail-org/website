...
默认情况下，chart 会自动生成 dashboard 会话签名密钥和加密密钥（`KUBETAIL_DASHBOARD_SESSION_SIGNING_KEY1` 和 `KUBETAIL_DASHBOARD_SESSION_ENCRYPTION_KEY1`），并将其存储在 Kubernetes Secret 中，以便在升级后保留。
...
| `kubetail.secrets.KUBETAIL_DASHBOARD_SESSION_SIGNING_KEY1` | string | `null` | Hex 编码的 dashboard 会话签名密钥（为 `null` 时自动生成） |
| `kubetail.secrets.KUBETAIL_DASHBOARD_SESSION_ENCRYPTION_KEY1` | string | `null` | Hex 编码的 dashboard 会话加密密钥（为 `null` 时自动生成） |
| `kubetail.secrets.KUBETAIL_DASHBOARD_SESSION_SIGNING_KEY2` | string | `null` | 用于轮换的上一个槽位签名密钥（Hex 编码，无自动生成） |
| `kubetail.secrets.KUBETAIL_DASHBOARD_SESSION_ENCRYPTION_KEY2` | string | `null` | 用于轮换的上一个槽位加密密钥（Hex 编码，无自动生成） |
...
| `kubetail.dashboard.runtimeConfig` | map | *see values.yaml* | Dashboard 运行时配置（参见 [Dashboard 参考](/zh-cn/reference/dashboard)） |
...
| `kubetail.dashboard.podTemplate.imagePullSecrets` | array | `[]` | 镜像拉取 secret |
...
| `kubetail.clusterAPI.runtimeConfig` | map | *see values.yaml* | Cluster API 运行时配置（参见 [Cluster API 参考](/zh-cn/reference/cluster-api)） |
...
| `kubetail.clusterAPI.podTemplate.imagePullSecrets` | array | `[]` | 镜像拉取 secret |
...
| `kubetail.clusterAgent.runtimeConfig` | map | *see values.yaml* | Cluster Agent 运行时配置（参见 [Cluster Agent 参考](/zh-cn/reference/cluster-agent)） |
...
| `kubetail.clusterAgent.podTemplate.imagePullSecrets` | array | `[]` | 镜像拉取 secret |
...
| `kubetail.clusterAgent.rbac.name` | string | `null` | 覆盖 RBAC 资源名称 |
| `kubetail.clusterAgent.rbac.annotations` | map | `{}` | 额外 RBAC annotation |
| `kubetail.clusterAgent.rbac.labels` | map | `{}` | 额外 RBAC label |
