...
安装说明请参阅[桌面端安装指南](/zh-cn/guides/desktop/installation)。

### 集群内

整套 Kubetail 都可以通过 Helm 或 YAML 清单作为 Kubernetes 资源安装在集群内部。在这种拓扑中，Dashboard Server 作为 `kubetail` namespace 中的一个 Deployment 运行，并通过 `kubectl port-forward`、`kubectl proxy` 或 ingress 资源从浏览器访问。这是共享团队环境中的首选方案，或者适用于您希望获得持久、常开访问，而不依赖本地 CLI 的情况。

安装说明请参阅[集群安装指南](/zh-cn/guides/cluster/installation)。
...
关于 Docker 部署方式的更多信息，请参阅[Docker 介绍](/zh-cn/guides/docker/introduction)。
...
完整命令和参数列表请参阅 [CLI 参考](/zh-cn/reference/cli)。
...
配置选项请参阅 [Dashboard 参考](/zh-cn/reference/dashboard)。
...
配置选项请参阅 [Cluster API 参考](/zh-cn/reference/cluster-api)。
...
配置选项请参阅 [Cluster Agent 参考](/zh-cn/reference/cluster-agent)。
...
<Aside type="tip">
若想深入了解每种拓扑的性能影响，请参阅 [Performance](/zh-cn/concepts/performance) 页面。
</Aside>
