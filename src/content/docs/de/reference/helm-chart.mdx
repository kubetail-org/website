...
Standardmaessig erzeugt das Chart die Signier- und Verschluesselungsschluessel der Dashboard-Session (`KUBETAIL_DASHBOARD_SESSION_SIGNING_KEY1` und `KUBETAIL_DASHBOARD_SESSION_ENCRYPTION_KEY1`) automatisch und speichert sie in einem Kubernetes-Secret, sodass sie bei Upgrades erhalten bleiben.
...
| `kubetail.secrets.KUBETAIL_DASHBOARD_SESSION_SIGNING_KEY1` | string | `null` | Hex-codierter Signierschluessel der Dashboard-Session (wird erzeugt, wenn `null`) |
| `kubetail.secrets.KUBETAIL_DASHBOARD_SESSION_ENCRYPTION_KEY1` | string | `null` | Hex-codierter Verschluesselungsschluessel der Dashboard-Session (wird erzeugt, wenn `null`) |
| `kubetail.secrets.KUBETAIL_DASHBOARD_SESSION_SIGNING_KEY2` | string | `null` | Hex-codierter Signierschluessel des vorherigen Slots zur Rotation (keine automatische Erzeugung) |
| `kubetail.secrets.KUBETAIL_DASHBOARD_SESSION_ENCRYPTION_KEY2` | string | `null` | Hex-codierter Verschluesselungsschluessel des vorherigen Slots zur Rotation (keine automatische Erzeugung) |
...
| `kubetail.dashboard.podTemplate.imagePullSecrets` | array | `[]` | Image-Pull-Secrets |
...
| `kubetail.clusterAPI.podTemplate.imagePullSecrets` | array | `[]` | Image-Pull-Secrets |
...
| `kubetail.clusterAgent.podTemplate.imagePullSecrets` | array | `[]` | Image-Pull-Secrets |
...
| `kubetail.clusterAgent.rbac.name` | string | `null` | Namen der RBAC-Ressourcen ueberschreiben |
| `kubetail.clusterAgent.rbac.annotations` | map | `{}` | Zusaetzliche RBAC-Annotationen |
| `kubetail.clusterAgent.rbac.labels` | map | `{}` | Zusaetzliche RBAC-Labels |
