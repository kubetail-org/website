...
Installationsanweisungen finden Sie im [Desktop-Installationsleitfaden](/de/guides/desktop/installation).

### Cluster

Der gesamte Kubetail-Stack kann mit Helm oder YAML-Manifests als Kubernetes-Ressourcen innerhalb Ihres Clusters installiert werden. In dieser Topologie läuft der Dashboard Server als Deployment im Namespace `kubetail` und wird im Browser über `kubectl port-forward`, `kubectl proxy` oder eine Ingress-Ressource aufgerufen. Das ist der bevorzugte Ansatz für gemeinsam genutzte Teamumgebungen oder wenn Sie dauerhaften, ständig verfügbaren Zugriff möchten, ohne sich auf eine lokale CLI zu verlassen.

Installationsanweisungen finden Sie im [Cluster-Installationsleitfaden](/de/guides/cluster/installation).
...
Weitere Informationen zu Docker-Deployment-Optionen finden Sie in der [Docker-Einführung](/de/guides/docker/introduction).
...
Eine vollständige Liste aller Befehle und Flags finden Sie in der [CLI-Referenz](/de/reference/cli).
...
Konfigurationsoptionen finden Sie in der [Dashboard-Referenz](/de/reference/dashboard).
...
Konfigurationsoptionen finden Sie in der [Cluster-API-Referenz](/de/reference/cluster-api).
...
Konfigurationsoptionen finden Sie in der [Cluster-Agent-Referenz](/de/reference/cluster-agent).
...
<Aside type="tip">
Einen tieferen Einblick in die Performance-Auswirkungen der einzelnen Topologien finden Sie auf der Seite [Performance](/de/concepts/performance).
</Aside>
