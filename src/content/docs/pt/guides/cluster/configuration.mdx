...
O Dashboard e o servidor web que hospeda a interface e faz proxy das requisicoes do navegador para o cluster. No ambiente de cluster, ele e executado como um Deployment no namespace `kubetail`.
...
A Cluster API e o servidor backend que faz proxy das requisicoes do Dashboard para as instancias do Cluster Agent em execucao em cada no. No cluster, ela e executada como um Deployment no namespace `kubetail`.
...
O Cluster Agent e o pequeno programa baseado em Rust que roda em cada no e le diretamente os arquivos de log de contêineres a partir do sistema de arquivos do no. No cluster, ele e executado como um DaemonSet no namespace `kubetail`.
