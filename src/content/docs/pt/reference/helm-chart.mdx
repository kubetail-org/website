...
Por padrao, o chart gera automaticamente as chaves de assinatura e criptografia da sessao do dashboard (`KUBETAIL_DASHBOARD_SESSION_SIGNING_KEY1` e `KUBETAIL_DASHBOARD_SESSION_ENCRYPTION_KEY1`) e as armazena em um Kubernetes Secret para que sejam preservadas durante upgrades.
...
| `kubetail.secrets.KUBETAIL_DASHBOARD_SESSION_SIGNING_KEY1` | string | `null` | Chave de assinatura da sessao do dashboard codificada em hex (gerada automaticamente se `null`) |
| `kubetail.secrets.KUBETAIL_DASHBOARD_SESSION_ENCRYPTION_KEY1` | string | `null` | Chave de criptografia da sessao do dashboard codificada em hex (gerada automaticamente se `null`) |
| `kubetail.secrets.KUBETAIL_DASHBOARD_SESSION_SIGNING_KEY2` | string | `null` | Chave de assinatura do slot anterior para rotacao codificada em hex (sem geracao automatica) |
| `kubetail.secrets.KUBETAIL_DASHBOARD_SESSION_ENCRYPTION_KEY2` | string | `null` | Chave de criptografia do slot anterior para rotacao codificada em hex (sem geracao automatica) |
...
| `kubetail.dashboard.podTemplate.imagePullSecrets` | array | `[]` | Secrets de pull de imagem |
...
| `kubetail.clusterAPI.podTemplate.imagePullSecrets` | array | `[]` | Secrets de pull de imagem |
...
| `kubetail.clusterAgent.podTemplate.imagePullSecrets` | array | `[]` | Secrets de pull de imagem |
...
| `kubetail.clusterAgent.rbac.name` | string | `null` | Sobrescrever os nomes dos recursos RBAC |
| `kubetail.clusterAgent.rbac.annotations` | map | `{}` | Annotations RBAC adicionais |
| `kubetail.clusterAgent.rbac.labels` | map | `{}` | Labels RBAC adicionais |
