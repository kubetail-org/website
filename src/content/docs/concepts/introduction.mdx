...
For more advanced features like log file sizes, last event timestamps, and search, you can install the **Kubetail API** into your cluster. It consists of an API server and a small cluster agent running on each node (see [Architecture](/concepts/architecture) for more details). The cluster agent is written in Rust and designed to be extremely performant (~3MB memory, ~4.00m CPU). Kubetail also supports multiple deployment options -- you can run the web dashboard in your cluster and expose it with an ingress, or run the CLI in a Docker container on your desktop or inside a cluster (see [Deployment Topologies](/concepts/architecture#deployment-topologies)). 
