...
To start using Kubetail in development, just install the `kubetail` CLI and run `kubetail serve` to view logs in your browser, or `kubetal logs` to stream logs to your terminal (see [Quickstart](/tutorials/getting-started/quickstart)). No cluster-side installation is required, and no log forwarding pipeline needs to be configured first.
